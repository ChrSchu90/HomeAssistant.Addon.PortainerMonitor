...
dotnet publish ${NET_PROJECT} -r ${NET_RUNTIME} -p:DebugType=embedded -p:PublishSingleFile=true --verbosity normal --configuration Release --self-contained -o ${NET_OUT} && \
  docker build --progress=plain --pull --rm --build-arg BUILD_VERSION="${BUILD_VERSION}" --build-arg BUILD_ARCH=${BUILD_ARCH} --build-arg BUILD_FROM=${BUILD_FROM} -f ${DOCKER_FILE} -t ${IMAGE_NAME} . && \
